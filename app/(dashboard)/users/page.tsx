import Link from 'next/link';
import {
  Plus, Pencil,
  ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight,
  UserRound,
} from 'lucide-react';
import type { Prisma } from '@prisma/client';
import { auth } from '@/auth';
import { db } from '@/lib/db';
import UserSearch      from './UserSearch';
import UserPerPage     from './UserPerPage';
import DeleteUserButton from './DeleteUserButton';

const PER_OPTIONS = [10, 25, 50];

export default async function UsersPage({
  searchParams,
}: {
  searchParams: Promise<{ q?: string; page?: string; per?: string }>;
}) {
  const [sp, session] = await Promise.all([searchParams, auth()]);

  const currentUserId = session?.user?.id ? parseInt(session.user.id, 10) : -1;

  const q         = sp.q?.trim() ?? '';
  const page      = Math.max(1, parseInt(sp.page ?? '1', 10));
  const perParsed = parseInt(sp.per ?? '10', 10);
  const perPage   = PER_OPTIONS.includes(perParsed) ? perParsed : 10;

  const where: Prisma.UserWhereInput = {
    deletedAt: null,
    ...(q && {
      OR: [
        { firstName:   { contains: q } },
        { lastName:    { contains: q } },
        { email:       { contains: q } },
        { role:        { contains: q } },
        { phoneNumber: { contains: q } },
      ],
    }),
  };

  const [total, users] = await Promise.all([
    db.user.count({ where }),
    db.user.findMany({
      where,
      orderBy: { createdAt: 'desc' },
      skip: (page - 1) * perPage,
      take: perPage,
      select: {
        id:          true,
        firstName:   true,
        lastName:    true,
        email:       true,
        phoneNumber: true,
        image:       true,
        role:        true,
        createdAt:   true,
      },
    }),
  ]);

  const totalPages = Math.max(1, Math.ceil(total / perPage));
  const from       = total === 0 ? 0 : (page - 1) * perPage + 1;
  const to         = Math.min(page * perPage, total);

  function pageUrl(p: number) {
    const params = new URLSearchParams();
    if (q) params.set('q', q);
    params.set('per',  String(perPage));
    params.set('page', String(Math.max(1, Math.min(p, totalPages))));
    return `/users?${params}`;
  }

  return (
    <>
      {/* ── toolbar ── */}
      <div className="gg-table-toolbar">
        <UserSearch defaultQ={q} />
        <div className="gg-spacer" />
        <Link href="/users/create" className="gg-btn gg-btn--primary">
          <Plus size={17} /> Create User
        </Link>
      </div>

      <div className="gg-card gg-card-pad">
        {/* ── table ── */}
        <div className="gg-table-wrap">
          <table className="gg-table">
            <thead>
              <tr>
                <th>User</th>
                <th>Role</th>
                <th>Phone Number</th>
                <th>Created On</th>
                <th style={{ textAlign: 'right' }}>Action</th>
              </tr>
            </thead>
            <tbody>
              {users.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: 0, border: 'none' }}>
                    <div className="gg-empty-state">
                      <UserRound size={42} style={{ color: 'var(--gray-300)' }} />
                      <p>{q ? `No users match "${q}".` : 'No users yet.'}</p>
                      {!q && (
                        <Link
                          href="/users/create"
                          className="gg-btn gg-btn--primary"
                          style={{ marginTop: 'var(--sp-2)' }}
                        >
                          <Plus size={16} /> Create your first user
                        </Link>
                      )}
                    </div>
                  </td>
                </tr>
              ) : (
                users.map((u) => {
                  const initials = `${u.firstName.charAt(0)}${u.lastName.charAt(0)}`.toUpperCase();
                  return (
                    <tr key={u.id}>
                      {/* User — avatar + name + email */}
                      <td>
                        <div className="ppl-name">
                          <div className="ppl-ava">
                            {u.image
                              ? <img src={u.image} alt={initials} />
                              : initials}
                          </div>
                          <div className="ppl-cell">
                            <span className="nm">{u.firstName} {u.lastName}</span>
                            <span className="em">{u.email}</span>
                          </div>
                        </div>
                      </td>

                      {/* Role — capitalize first letter */}
                      <td>
                        {u.role.charAt(0).toUpperCase() + u.role.slice(1)}
                      </td>

                      {/* Phone Number */}
                      <td className="gg-num">
                        {u.phoneNumber ?? <span className="gg-muted">—</span>}
                      </td>

                      {/* Created On */}
                      <td>
                        <span className="gg-chip-time gg-num">
                          {u.createdAt.toLocaleTimeString('en-US', {
                            hour: '2-digit', minute: '2-digit', timeZone: 'UTC',
                          })}
                          <br />
                          {u.createdAt.toLocaleDateString('en-US', {
                            month: '2-digit', day: '2-digit', year: 'numeric', timeZone: 'UTC',
                          })}
                        </span>
                      </td>

                      {/* Actions */}
                      <td style={{ textAlign: 'right' }}>
                        <div className="row-acts">
                          <Link
                            href={`/users/${u.id}/edit`}
                            className="act-edit"
                            title="Edit"
                          >
                            <Pencil size={17} />
                          </Link>
                          {/* Current user's own row has no delete button */}
                          <DeleteUserButton
                            id={u.id}
                            name={`${u.firstName} ${u.lastName}`}
                            isCurrentUser={u.id === currentUserId}
                          />
                        </div>
                      </td>
                    </tr>
                  );
                })
              )}
            </tbody>
          </table>
        </div>

        {/* ── pagination ── */}
        <div className="gg-pagination">
          <div className="gg-perpage">
            <span className="gg-muted">Records per page</span>
            <UserPerPage perPage={perPage} />
          </div>

          <span className="gg-muted gg-num">{from}–{to} of {total}</span>

          <div className="gg-spacer" />

          <Link href={pageUrl(1)}          className="gg-page-btn" aria-label="First"><ChevronsLeft  size={17} /></Link>
          <Link href={pageUrl(page - 1)}   className="gg-page-btn" aria-label="Previous"><ChevronLeft   size={17} /></Link>
          <Link href={pageUrl(page + 1)}   className="gg-page-btn" aria-label="Next"><ChevronRight  size={17} /></Link>
          <Link href={pageUrl(totalPages)} className="gg-page-btn" aria-label="Last"><ChevronsRight size={17} /></Link>
        </div>
      </div>

      <div className="dash-foot">
        All Rights Reserved &copy; 2026 <a href="#">GildedGlow</a>
      </div>
    </>
  );
}
