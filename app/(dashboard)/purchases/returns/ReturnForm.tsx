...
import type { ActionResult, SearchProductForReturn } from './actions';
...
export type LineItem = {
  ...
  code:         string;
  productUnit:  string;
  // currentStock is the live warehouse stock shown in the table for reference.
  currentStock: number;
  netUnitCost:  number;
  quantity:     number;
...
export default function ReturnForm({ warehouses, suppliers, units, initial }: Props) {
  const router = useRouter();
  const isEdit = !!initial?.id;

  const [state, formAction, isPending] = useActionState<ActionResult, FormData>(
    isEdit ? updatePurchaseReturn : createPurchaseReturn,
    {},
  );
  ...
  const [results,     setResults]     = useState<SearchProductForReturn[]>([]);
  ...
  useEffect(() => {
    ...
    if (state.success) router.push(initialId ? `/purchases/returns/${initialId}` : '/purchases/returns');
  ...
  function handleQueryChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    searchTimer.current = setTimeout(() => {
      startSearch(async () => {
        const res = await searchProductsForReturn(q.trim(), warehouseId || null);
        setResults(res);
        setShowResults(true);
      });
    }, 300);
  ...
  function addItem(product: SearchProductForReturn) {
    ...
    setItems((prev) => [
      ...prev,
      {
        ...
        taxType:      'Exclusive',
        orderTax:     0,
        returnUnit:   product.productUnit,
      },
    ]);
    setQuery('');
    setResults([]);
  ...
  // ── Serialised line items for hidden field ────────────────────────────────
  const itemsJson = JSON.stringify(
    items.map(({ productId, quantity, discountType, discount, taxType, orderTax, netUnitCost, returnUnit }) => ({
      productId, quantity, discountType, discount, taxType, orderTax, netUnitCost, returnUnit,
    })),
  );
        ...
        >
          <div className="gg-modal">
            ...
            <div className="gg-modal-body">
              ...
              <div className="gg-field">
                <label className="gg-label">Return Unit <span className="gg-req">*</span></label>
                <select
                  className="gg-select"
                  value={modalUnit}
