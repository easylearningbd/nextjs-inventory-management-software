...
export default async function PurchaseReturnPDFPage({
  params,
}: {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--canvas)', padding: 'var(--sp-6)' }}>
      ...
      <div className="page-head pur-noprint" style={{ marginBottom: 'var(--sp-6)' }}>
        <h1 className="gg-page-title">Purchase Return PDF</h1>
      ...
      <div className="gg-card gg-card-pad">
        ...
        <div className="pur-info-grid">
          ...
          <div className="pur-info-panel">
            ...
            <div className="pur-info-body">
              <div className="pur-info-row"><User size={17} /><span>{sup.name}</span></div>
              {sup.email && (
                <div className="pur-info-row"><Mail size={17} /><span>{sup.email}</span></div>
              )}
              {sup.phoneNumber && (
                <div className="pur-info-row"><Smartphone size={17} /><span className="gg-num">{sup.phoneNumber}</span></div>
              )}
              <div className="pur-info-row"><MapPin size={17} /><span>{supAddress}</span></div>
            </div>
          </div>
          ...
          <div className="pur-info-panel">
            <div className="pur-band">Return Info</div>
            <div className="pur-info-body">
              ...
              <div className="pur-info-line">
                ...
                <span className="v"><StatusBadge status={ret.status} /></span>
              </div>
              <div className="pur-info-line">
              ...
              <div className="pur-info-line">
                ...
                <span className="v">{ret.paymentType}</span>
              </div>
              <div className="pur-info-line">
                <span className="k">Paid :</span>
                <span className="v gg-num">{fmt(paid)}</span>
              </div>
              <div className="pur-info-line">
                <span className="k">Due :</span>
                <span className="v gg-num">{fmt(due)}</span>
              </div>
            </div>
          </div>
          ...
          <table className="gg-table">
            ...
            <tbody>
              {ret.items.map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    <td className="gg-td-strong">{item.product.code} ({item.product.name})</td>
        ...
        <div className="pur-totals-box">
          ...
          <div className="pur-totals-row">
            ...
            <span className="ptr-val gg-num">{fmt(paid)}</span>
          </div>
          <div className="pur-totals-row">
            <span className="ptr-lbl">Due</span>
            <span className="ptr-val gg-num">{fmt(due)}</span>
          </div>
        </div>
